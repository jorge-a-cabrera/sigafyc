            ...
            If loDataRow.Count > 0 Then
                ...
                For Each loPropiedadInfo In moObjeto.GetType.GetProperties()
                    If LFsExisteAtributo(msRequeridos.ToLower, loPropiedadInfo.Name.ToLower) = sSi_ Then
                        If loDataRow.Item(0).Item(loPropiedadInfo.Name) IsNot DBNull.Value Then
                            loPropiedadInfo.SetValue(moObjeto, loDataRow.Item(0).Item(loPropiedadInfo.Name))
                        Else
                            Select Case sSeparador_ & loPropiedadInfo.PropertyType.Name
                                Case sString_
                                    loPropiedadInfo.SetValue(moObjeto, "")
                                Case sDateTime_
                                    loPropiedadInfo.SetValue(moObjeto, #01/01/1900#)
                                Case sDecimal_
                                    loPropiedadInfo.SetValue(moObjeto, 0.00D)
                                Case Else
                                    loPropiedadInfo.SetValue(moObjeto, 0)
                            End Select
                        End If
                    End If
                Next
            ...
            If loDataRow.Count > 0 Then
                ...
                If lsVerificacion = sOk_ Then
                    ...
                    If LFsExisteModificaciones(loBackTable, loDataTable) = sSi_ Then
                        If psConfirmar = sSi_ Then
                            If GFsConfirmacion(lsAccion, loDataRow.Item(0)) = sCancelar_ Then
                                If psAccion.Trim.Length > 0 Then Throw New ConfirmacionCancelado
                                Exit Sub
                            End If
                        End If
                        ActualizarTable(loDataTable)
            ...
            If InStr(lsResultado, sPrefijoParam_) > 0 Then
                If LFsExisteAtributo(msRequeridos.ToLower, loPropiedadInfo.Name.ToLower) = sSi_ Then
                    ...
                    If loPropiedadInfo.GetValue(moObjeto, Nothing) IsNot Nothing Then
                        ...
                        lsResultado = lsResultado.Replace(lsParametro, AsignarValor(loPropiedadInfo.GetValue(moObjeto, Nothing).ToString))
                    End If
                End If
            Else
                Exit For
